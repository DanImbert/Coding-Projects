...
def history():
    ...
    # Retrieve transaction history for the logged-in user from the database
...
def sell():
    ...
    # Query the database to get the symbols and shares owned by the user
    stocks = db.execute("SELECT symbol, SUM(shares) as total_shares FROM transactions WHERE user_id = :user_id GROUP BY symbol HAVING total_shares > 0",
        user_id=session["user_id"])
    ...
    if request.method == "POST":
        ...
        shares = request.form.get("shares")
        if not symbol:
            return apology("please select a valid stock symbol to sell")
        elif not shares or not shares.isdigit() or int(shares) <= 0:
        ...
        # If method is GET, render the form to sell shares
    else:
